-- Reading is always free (paywall.md Q-22); Pass unlocks premium content only.
-- @see docs/specs/monetization/paywall.md

ALTER TABLE public.story_monetization
  DROP COLUMN IF EXISTS requires_pass_to_read;
  ...
  SELECT (
    ...
    OR EXISTS (
      ...
      WHERE c.story_id = p_story_id
        AND cm.chapter_pass_tier_key IS NOT NULL
    )
    OR EXISTS (
